...
multi_cities = st.sidebar.multiselect("Select cities", sorted(CITIES.keys()), default=["Oslo", "Bergen"])
...
for city in multi_cities:
    ...
    try:
        ...
        df["Wind Chill (°C)"] = df.apply(lambda r: wind_chill(r["Temperature (°C)"], r["Wind Speed (km/h)"]), axis=1)
        city_dfs[city] = df
    ...
    for hour_idx in range(hours):
        ...
        if st.session_state.page == "Multi-city":
            ...
            if not compare_df.empty:
                fig_graph = px.line(compare_df, x="Time", y=compare_df.columns[1:], labels={"value":"°C","variable":"Metric"},
        ...
        if st.session_state.page == "Heatmap":
            z = pd.DataFrame({city: city_dfs[city]["Wind Chill (°C)"].iloc[:hour_idx+1] for city in multi_cities})
            fig_heatmap = px.imshow(z.T, labels=dict(x="Hour",y="City",color="Wind Chill (°C)"),
        ...
        if st.session_state.page == "Cold Meter":
            ...
            cols = 3
            rows = (n+cols-1)//cols
            fig_gauge = make_subplots(rows=rows, cols=cols, specs=[[{"type":"indicator"}]*cols]*rows,
            ...
            for idx, city in enumerate(multi_cities):
                ...
                fig_gauge.add_trace(go.Indicator(
                    ...
                    gauge={
                        'axis': {'range':[-40,10]},
                        'bar': {'color':color},
                        'steps':[{'range':[-40,-20],'color':'#0d3b66'},
        ...
        if st.session_state.page == "City Map":
            ...
            fig_map = go.Figure(go.Scattermapbox(
                lat=frame_df["Lat"], lon=frame_df["Lon"],
                mode="markers+text",
                marker=dict(size=16, color=frame_df["Wind Chill"], colorscale="RdBu_r",
                            cmin=-40,cmax=10,colorbar=dict(title="Wind Chill °C")),
                text=frame_df["City"], textposition="top center"
            ))
            fig_map.update_layout(mapbox_style="carto-positron", mapbox_zoom=4.5,
                                  mapbox_center={"lat":60,"lon":10}, height=400)
