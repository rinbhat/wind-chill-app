...
# Default Cities (NEW)
# -----------------------------
DEFAULT_CITIES = ["Oslo", "Stavanger", "Trondheim", "Bergen"]
...
if "page" not in st.session_state:
    # DEFAULT PAGE IS NOW MULTI-CITY
    st.session_state.page = "Multi-city"
...
if st.session_state.page == "Home":
    ...
    if multi_cities:
        ...
        leaderboard_df = pd.DataFrame({
            "City": multi_cities,
            "Temperature (°C)": [
                city_dfs[c]["Temperature (°C)"].iloc[latest_hour]
                for c in multi_cities
            ],
            "Wind Chill (°C)": [
                city_dfs[c]["Wind Chill (°C)"].iloc[latest_hour]
                for c in multi_cities
            ]
...
# -----------------------------
    ...
    for hour_idx in range(hours):
        ...
        # Multi-city Comparison (DEFAULT PAGE)
        if st.session_state.page == "Multi-city":
            ...
            for city in multi_cities:
                df = city_dfs[city][
                    ["Time","Temperature (°C)","Wind Chill (°C)"]
                ].iloc[:hour_idx+1]
        ...
        if st.session_state.page == "Cold Meter":
            ...
            fig_gauge.update_layout(
                height=250*rows,
                title_text=f"Cold Meter – Hour {hour_idx+1}"
            )
        ...
        if st.session_state.page == "City Map":
            ...
            frame_df = pd.DataFrame({
                ...
                "Wind Chill": [
                    city_dfs[c]["Wind Chill (°C)"].iloc[hour_idx]
                    for c in multi_cities
                ]
            })
